...
constexpr const char* TAG = "ldrweb";
...
LdrWebServer::LdrWebServer(WebContext* ctx, Settings& settings, const std::atomic<float>& lastLux)
    : WebServer(ctx), settings_(settings), lastLux_(lastLux) {}

esp_err_t LdrWebServer::start() {
...
void LdrWebServer::populate_healthz_fields(WebContext*, JsonWrapper& json) {
    ...
    const float lux = lastLux_.load(std::memory_order_relaxed);
    if (lux >= 0.0f) json.AddItem("lux", lux);
}

// POST /firmware — raw .bin body. Streams into the inactive OTA slot, sets it as
// the next boot partition, then reboots.
// Deploy: curl --data-binary @build/ldr3.bin http://<host>/firmware
esp_err_t LdrWebServer::firmware_post_handler(httpd_req_t* req) {
...
esp_err_t LdrWebServer::firmware_get_handler(httpd_req_t* req) {
    const esp_app_desc_t*  desc    = esp_app_get_description();
    const esp_partition_t* running = esp_ota_get_running_partition();
    JsonWrapper resp;
...
esp_err_t LdrWebServer::config_get_handler(httpd_req_t* req) {
...
esp_err_t LdrWebServer::config_reset_post_handler(httpd_req_t* req) {
    LdrWebServer* self = static_cast<LdrWebServer*>(req->user_ctx);
