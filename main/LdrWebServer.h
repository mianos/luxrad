...
// ldr3 HTTP control surface, layered on the shared WebServer base
...
class LdrWebServer : public WebServer {
public:
    LdrWebServer(WebContext* ctx, Settings& settings, const std::atomic<float>& lastLux);
...
private:
    ...
    Settings&                 settings_;
    const std::atomic<float>& lastLux_;
};
