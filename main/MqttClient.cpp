    ...
    std::vector<HandlerBinding> local_handlers = {
        {"cmnd/+/settings",
         ...
         [](MqttClient* client_ptr, const std::string& topic, cJSON* data) {
             ...
             auto& settings_ref = client_ptr->getSettings();
             auto changes = settings_ref.updateFromJsonWrapper(json_data);
             std::string ack = settings_ref.convertChangesToJson(changes);
             client_ptr->publish("tele/" + settings_ref.sensorName + "/settingsack", ack);
             ESP_LOGI(kMqttTag, "Applied settings for %s: %s",
                      topic.c_str(), ack.c_str());
         }}
    };

    for (const auto& handler : local_handlers) {
        subscribe(handler.subscriptionTopic);
        bindings.push_back({"", handler.matchPattern, handler.handler});
    }
}
